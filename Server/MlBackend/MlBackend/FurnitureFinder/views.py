...
async def scrape_website_links(url, channel_name, is_sitemap=False, custom_sitemap_tags=None, wanted_words=None,
                         output_file="data/scraped_links.csv"): # Method to run from other scripts
...
async def inference_on_link_with_response(link, channel_name):
    channel_layer = get_channel_layer()
    try:
...
async def inference_on_links(links,  channel_name, max_workers=32):
    channel_layer = get_channel_layer()
...
async def processing_user_request(link, channel_name, scrape_subpages=False, custom_sitemap_tags=None, wanted_words=None):
    if scrape_subpages is False:
        await inference_on_link_with_response(link, channel_name)
    else:
        is_sitemap = False
        if '.xml' in link:
            is_sitemap = True
